...
workingDirectory="/Users/williz/Desktop/ModelosED/3. Articulo 3/4. Resultados/Prospectiva/3Rutas/Modelo_1"
...
apollo_control = list(
  modelName  = "Modelo_1_Prospec_Simple_3Rutas",
...
for (i in 1:nrow(database)){ 
  ...
  delta = 0.69
  theta = 0
  eta   = 0.0        # 0.18 Estan Calibrados ... No Cambiar
  eta1  = 0.0          # Esta calibrado a cero.
  ...
  if(database$DT1[i] < -phi)
    ...
    {database$VP1[i]<- 0}
  if(database$DT1[i] <= sigma && database$DT1[i] >= -phi )
  {database$VN1[i] <-0 ; database$VP1[i]<- 0}
  ...
  if(database$DT2[i] < -phi)
    ...
    {database$VP2[i]<- 0}
  if (database$DT2[i] <= sigma && database$DT2[i] >= -phi )
    {database$VN2[i] <-0 ; database$VP2[i]<- 0}
  ...
  if(database$DT4[i] < -phi )
    ...
    {database$VP4[i]<- 0}
  if (database$DT4[i] <= sigma && database$DT4[i] >= -phi )
    {database$VN4[i] <-0 ; database$VP4[i]<- 0}
}
...
## Función de ponderación  Prelec

for (i in 1:nrow(database)){
  # Pi_Alt_1
  if (database$T_Alt_4[i] < database$T_Alt_1[i])
  {if (database$P_Alt1[i] == B1)
  {database$Pi_Alt1[i] = exp(-(-log(B1))**gamma)}
    if (database$P_Alt1[i] == B2)
    {database$Pi_Alt1[i] = exp(-(-log(B1+B2))**gamma) - exp(-(-log(B1))**gamma)}}
  else {if (database$P_Alt1[i] == C1)
  {database$Pi_Alt1[i] = exp(-(-log(C1))**gamma)}
    if (database$P_Alt1[i] == C2)
    {database$Pi_Alt1[i] = exp(-(-log(C1+C2))**gamma) - exp(-(-log(C1))**gamma)}}
  
  ## Pi_Alt_2
  if (database$T_Alt_4[i] < database$T_Alt_2[i])
  {if (database$P_Alt2[i] == B1)
  {database$Pi_Alt2[i] = exp(-(-log(B1))**gamma)}
    if (database$P_Alt2[i] == B2)
    {database$Pi_Alt2[i] = exp(-(-log(B1+B2))**gamma) - exp(-(-log(B1))**gamma)}}
  else {if (database$P_Alt2[i] == C1)
  {database$Pi_Alt2[i] = exp(-(-log(C1))**gamma)}
    if (database$P_Alt2[i] == C2)
    {database$Pi_Alt2[i] = exp(-(-log(C1+C2))**gamma) - exp(-(-log(C1))**gamma)}}
  
  ## Pi_Alt_4
  if (database$T_Alt_4[i] < database$T_rf_Alt4[i])
  {if (database$P_Alt4[i] == B1)
  {database$Pi_Alt4[i] = exp(-(-log(B1))**gamma)}
    if (database$P_Alt4[i] == B2)
    {database$Pi_Alt4[i] = exp(-(-log(B1+B2))**gamma) - exp(-(-log(B1))**gamma)}}
  else {if (database$P_Alt1[i] == C1)
  {database$Pi_Alt4[i] = exp(-(-log(C1))**gamma)}
    if (database$P_Alt4[i] == C2)
    {database$Pi_Alt4[i] = exp(-(-log(C1+C2))**gamma) - exp(-(-log(C1))**gamma)}}
}
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[['ruta1']]  = asc_ruta1  + 
    b_gain*(Pi_Alt1*VP1*(1 + CG_Alt_1)) + b_loss*(Pi_Alt1*VN1*(1 + CG_Alt_1)) + 
  ...
  V[['ruta2']]  = asc_ruta2 + 
    b_gain*(Pi_Alt2*VP2*(1 + CG_Alt_2)) + b_loss*(Pi_Alt2*VN2*(1 + CG_Alt_2)) + 
  ...
  V[['rutaEC']] =  asc_ruta4 + 
    b_gain*(Pi_Alt4*VP4*(1 + CG_Alt_4)) + b_loss*(Pi_Alt4*VN4*(1 + CG_Alt_4)) + 
...
database$V1 = database$VP1 + database$VI1 + database$VN1
database$V2 = database$VP2 + database$VI2 + database$VN2
database$V4 = database$VP4 + database$VI4 + database$VN4
plot(database$DT1,database$V1)
plot(database$DT2,database$V2)
plot(database$DT4,database$V4)
