...
apollo_control = list(
  modelName  = "MNL_Modelo_3Elecciones",
  modelDescr = "Modelo MNL_3Elecciones",
  indivID    = "ViajeId",
  nCores     = 1
)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[['ruta2']]  = asc_ruta2  + b_tt * min(T_Alt_2,T_Alt_3)*(1 + min(CG_Alt_2,CG_Alt_3)) + 
    ...
    b_NO_CAMFD * min(NO_CAMFD_A2, NO_CAMFD_A3) + b_SI_CAMFD * max(SI_CAMFD_A2, SI_CAMFD_A3) +
    b_NO_PANEL * min(NO_PANEL_A2, NO_PANEL_A3) + b_SI_PANEL * max(SI_PANEL_A2, SI_PANEL_A3) + 
    b_NO_ZER * min(NO_ZER_A2, NO_ZER_A3) + b_SI_ZER * max(SI_ZER_A2, SI_ZER_A3) + 
    b_No_MTRP * min(NO_MTRP_A2, NO_MTRP_A3) + b_Si_MTRP * max(SI_MTRP_A2, SI_MTRP_A3)
...
V_1 = ce[1]  + ce[3] * db$T_Alt_1*(1 + db$CG_Alt_1) + ce[4] * db$D_Alt_1 +
  ce[5] * db$SEM_A1_km + 
...
V_2 = ce[3] * min(db$T_Alt_2,db$T_Alt_3)*(1 + min(db$CG_Alt_2,db$CG_Alt_3)) + 
...
V_4 = ce[2] + ce[3] * db$T_Alt_4*(1 + db$CG_Alt_4) + ce[4] * db$D_Alt_4 + 
  ce[5] * db$SEM_EC_km +
