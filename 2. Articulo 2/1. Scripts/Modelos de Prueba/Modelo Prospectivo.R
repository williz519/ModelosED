...
# Tversky & Kahnelman
for(z in 1:nrow(database)){
  gama= 0.71 #Wu & Gonzalez gama=0.69 #Gao (2010) gama=0.59 #Camerer and Ho (1994)
  
  database$W_Pgain_Alt1[z] <- (database$Pgain_Alt1[z]**gama)/((database$Pgain_Alt1[z]**gama + (1-database$Pgain_Alt1)**gama)**(1/gama))
  database$W_Ploss_Alt1[z] <- (database$Ploss_Alt1[z]**gama)/((database$Ploss_Alt1[z]**gama + (1-database$Ploss_Alt1)**gama)**(1/gama))
  
  database$W_Pgain_Alt2[z] <- (database$Pgain_Alt2[z]**gama)/((database$Pgain_Alt2[z]**gama + (1-database$Pgain_Alt2)**gama)**(1/gama))
  database$W_Ploss_Alt2[z] <- (database$Ploss_Alt2[z]**gama)/((database$Ploss_Alt2[z]**gama + (1-database$Ploss_Alt2)**gama)**(1/gama))
  
  database$W_Pgain_Alt3[z] <- (database$Pgain_Alt3[z]**gama)/((database$Pgain_Alt3[z]**gama + (1-database$Pgain_Alt3)**gama)**(1/gama))
  database$W_Ploss_Alt3[z] <- (database$Ploss_Alt3[z]**gama)/((database$Ploss_Alt3[z]**gama + (1-database$Ploss_Alt3)**gama)**(1/gama))
  
  database$W_Pgain_Alt4[z] <- (database$Pgain_Alt4[z]**gama)/((database$Pgain_Alt4[z]**gama + (1-database$Pgain_Alt4)**gama)**(1/gama))
  database$W_Ploss_Alt4[z] <- (database$Ploss_Alt4[z]**gama)/((database$Ploss_Alt4[z]**gama + (1-database$Ploss_Alt4)**gama)**(1/gama))
}
...
for(z in 1:nrow(database)){
  ...
  database$W_Pgain_Alt1[z] <- (Pgain_Alt1[z]**gama)/((Pgain_Alt1[z]**gama + (1-Pgain_Alt1)**gama)**tao)
  database$W_Ploss_Alt1[z] <- (Ploss_Alt1[z]**gama)/((Ploss_Alt1[z]**gama + (1-Ploss_Alt1)**gama)**tao)
  
  database$W_Pgain_Alt2[z] <- (Pgain_Alt2[z]**gama)/((Pgain_Alt2[z]**gama + (1-Pgain_Alt2)**gama)**tao)
  database$W_Ploss_Alt2[z] <- (Ploss_Alt2[z]**gama)/((Ploss_Alt2[z]**gama + (1-Ploss_Alt2)**gama)**tao)
  
  database$W_Pgain_Alt3[z] <- (Pgain_Alt3[z]**gama)/((Pgain_Alt3[z]**gama + (1-Pgain_Alt3)**gama)**tao)
  database$W_Ploss_Alt3[z] <- (Ploss_Alt3[z]**gama)/((Ploss_Alt3[z]**gama + (1-Ploss_Alt3)**gama)**tao)
  
  database$W_Pgain_Alt4[z] <- (Pgain_Alt4[z]**gama)/((Pgain_Alt4[z]**gama + (1-Pgain_Alt4)**gama)**tao)
  database$W_Ploss_Alt4[z] <- (Ploss_Alt4[z]**gama)/((Ploss_Alt4[z]**gama + (1-Ploss_Alt4)**gama)**tao)
}
...
#Prelec II
for(z in 1:nrow(database)){
  gama= 1.42 # Hensher at al (2011), gama=0.53 tao=1.08 Bleichrodt and Pinto (2000)
  tao = 0.73
  
  database$W_Pgain_Alt1[z] <- exp(-tao*(-log(Pgain_Alt1[z])**gama))
  database$W_Ploss_Alt1[z] <- exp(-tao*(-log(Ploss_Alt1[z])**gama))
  
  database$W_Pgain_Alt2[z] <- exp(-tao*(-log(Pgain_Alt2[z])**gama))
  database$W_Ploss_Alt2[z] <- exp(-tao*(-log(Ploss_Alt2[z])**gama))
  
  database$W_Pgain_Alt3[z] <- exp(-tao*(-log(Pgain_Alt3[z])**gama))
  database$W_Ploss_Alt3[z] <- exp(-tao*(-log(Ploss_Alt3[z])**gama))
  
  database$W_Pgain_Alt4[z] <- exp(-tao*(-log(Pgain_Alt4[z])**gama))
  database$W_Ploss_Alt4[z] <- exp(-tao*(-log(Ploss_Alt4[z])**gama))
}
