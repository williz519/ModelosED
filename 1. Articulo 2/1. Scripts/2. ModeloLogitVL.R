...
DBCompleta <- read_xlsx("/Users/williz/Desktop/ModelosED/Bases de Datos Ordenada/DBViajesCompleta.xlsx")
...
Datos <- RutasGoogle %>%
  ...
  inner_join(DBCompleta %>%
               select(ViajeId,Semaf_A1:MtrP_EC,MODELO,Acc_EC:Acc_A3,
                      Paneles_EC:Paneles_A3),
             by = "ViajeId") %>%
  # Modo conducción
...
DBModelo <- Datos %>%
  select("ViajeId", "GENERO", "TIEMPO_PROFESION", "HORAS_TRABAJO","NIVEL_EDUCATIVO",
         ...
         "Paneles_EC", "Paneles_A1", "Paneles_A2", "Paneles_A3" , "MODELO",
...
DBModelo = read.csv("/Users/williz/Desktop/ModelosED/Database/ModeloLogitVL.csv", sep="\t", dec=".")
...
MCond$Acc_A1 <- as.numeric(MCond$Acc_A1)
MCond$MtrP_A3[is.na(MCond$MtrP_A3)] <- 999
