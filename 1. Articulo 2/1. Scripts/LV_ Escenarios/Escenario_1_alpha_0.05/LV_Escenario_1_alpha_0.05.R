...
sink("Modelo_Dos_Factores_alpha_005.txt")

# MODELO 1 Dos Factores alpha 0.05 t value= 1.96
...
# Se elimina la variable UsoCel del modelo por aparecer muchas veces para correlacionar 

model1_6 <- " #Variables Latentes
      FA_1 =~ FRbr + EnfCond + AFrSem + CulFr + OmLmVel + IgPare 
      FA_2 =~ PasoPeaton + UsoDirec 
...
fit1_9 <- cfa(model1_9, data = MCond, orthogonal = TRUE)
summary(fit1_9, fit.measures= TRUE, standardized = TRUE)
semPaths(fit1_9, what = "std", style = "mx", title = FALSE, curvePivot = TRUE)
#parameterestimates(fit11)
modindices(fit1_9, sort = TRUE, maximum.number = 10)
...
fit_index07 <- broom::glance(fit1_7) %>% 
  select(chisq, npar, cfi, tli, rmsea, rmsea.conf.high, srmr, aic, bic, estimator)

fit_index08 <- broom::glance(fit1_8) %>% 
  select(chisq, npar, cfi, tli, rmsea, rmsea.conf.high, srmr, aic, bic, estimator)

fit_index09 <- broom::glance(fit1_9) %>% 
  select(chisq, npar, cfi, tli, rmsea, rmsea.conf.high, srmr, aic, bic, estimator)
...
sink("Modelo_Tres_Factores_alpha_005.txt")
...
FA_2 ~ CONG_CD + USOCINTURON 

FA_3 ~ EDUBASICA + EXP_2 + USOCINTURON + USODISPMOB
...
FA_1 ~ USOCINTURON
FA_2 ~ CONG_CD + USOCINTURON 
FA_3 ~ EDUBASICA + USOCINTURON + USODISPMOB
FA_2 ~ FA_1
...
fit2_6 <- cfa(model2_6, data = MCond, orthogonal = TRUE)
summary(fit2_6, fit.measures=TRUE, standardized = TRUE)
semPaths(fit2_6,"std", title = FALSE, curvePivot = TRUE)
# Solicitar los 10 primeros IM con valores m?s altos
modindices(fit2_6, sort = TRUE, maximum.number = 10)


model2_7 <- " #Variables Latentes
      FA_1 =~ FRbr + EnfCond + AFrSem + CulFr 
      FA_2 =~ OmLmVel + IgPare + UsoCel 
      FA_3 =~ PasoPeaton + UsoDirec + UsoCel
...
fit2_7 <- cfa(model2_7, data = MCond, orthogonal = TRUE)
summary(fit2_7, fit.measures=TRUE, standardized = TRUE)
semPaths(fit2_7,"std", title = FALSE, curvePivot = TRUE)
# Solicitar los 10 primeros IM con valores m?s altos
modindices(fit2_7, sort = TRUE, maximum.number = 10)
...
fit_index06 <- broom::glance(fit2_6) %>% 
  select(chisq, npar, cfi, tli, rmsea, rmsea.conf.high, srmr, aic, bic, estimator)

fit_index07 <- broom::glance(fit2_7) %>% 
  select(chisq, npar, cfi, tli, rmsea, rmsea.conf.high, srmr, aic, bic, estimator)


# Uniendo 
bind_rows(fit_index00, fit_index01, fit_index02, fit_index03, fit_index04, fit_index05, fit_index06, 
