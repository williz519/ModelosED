...
class Map:
    ''' Clase utilizada para manejar la información proporcionada por el mapa de entrada '''
    class Nodes:
        ''' Clase para representar los nodos utilizados por el algoritmo ACO '''
        ...
        def compute_edges(self,map_arr):
            ''' clase que devuelve los bordes conectados a cada nodo '''
    ...
    def _create_nodes(self):
        ''' Crear nodos fuera del mapa inicial '''
    ...
    def _read_map(self, map_name):
        ''' Lee datos de un archivo txt de mapa de entrada'''
        map_planning = np.loadtxt('./maps/' + map_name, dtype = 'string')
        return map_planning

    def _map_2_occupancy_map(self):
        ''' Toma la matriz y la convierte en una matriz flotante '''
